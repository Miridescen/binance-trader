...
LEVERAGE             = 3
MARGIN_PER_POS       = 10
TOP_N_SHORT          = 10       # 涨幅榜空单最多开仓数
TOP_N_LOSER_SHORT    = 10       # 跌幅榜空单最多开仓数
TOP_N_LONG           = 10       # 多单最多开仓数（仅虚拟盘）
TOP_N                = max(TOP_N_SHORT, TOP_N_LONG)  # 兼容旧引用
MIN_CHANGE_SHORT     = 5.0      # 空单入场最低涨幅（%）
MIN_CHANGE_LONG      = 8.0      # 多单入场最低跌幅（%）/ 跌幅榜空单入场最低跌幅（%）
...
def _fill_close_time(symbols: list):
    ...
    for sym in symbols:
        row = db.get_oldest_open_position(sym)
        if not row:
            continue
        ...
        try:
            ...
            db.update_close_data(sym, "", {"close_time": ts})
            continue

        pnl, roe = _calc_pnl_roe(row, mark)
        db.update_close_data(sym, "", {
            "close_time":     ts,
            "close_price":    mark,
...
def _calc_pnl_roe(row: dict, mark: float) -> tuple:
    """根据 open_log 行 + 当前 mark_price 计算 pnl 和 roe。
    side 含「空」按做空算，否则按做多。"""
    ...
    pnl = (entry - mark) * amt if is_short else (mark - entry) * amt
    roe = pnl / margin * 100 if margin else 0
    return pnl, roe
...
def run_open():
    ...
    n_loser = TOP_N_LOSER_SHORT * CANDIDATE_BUFFER
    gainer_pool = tickers[:n_short]
    loser_pool  = tickers[-n_loser:][::-1]
    ...
    log.info(f"跌幅榜空单候选：{len(top_losers)} 个（跌幅 ≥{MIN_CHANGE_LONG}%，上限 {TOP_N_LOSER_SHORT}）")
