...
def init_db():
    ...
    with get_conn() as conn:
        conn.executescript("""
        -- 基差快照：每 15 分钟一次，每个 (market, pair, contract) 一行
        CREATE TABLE IF NOT EXISTS basis_snapshot (
            id              INTEGER PRIMARY KEY AUTOINCREMENT,
            time            TEXT,           -- 采集时间 YYYY-MM-DD HH:MM:SS
            market          TEXT,           -- UM=U本位 / CM=币本位
            pair            TEXT,           -- 合约 pair: BTCUSDT / BTCUSD
            spot_symbol     TEXT,           -- 对标现货: BTCUSDT / ETHUSDT ...
            contract_type   TEXT,           -- CURRENT_QUARTER / NEXT_QUARTER
            contract_symbol TEXT,           -- BTCUSDT_260626 / BTCUSD_260925 等
            ...
            fut_vol_24h     REAL            -- 合约 24h 成交额（USDT 估算）
        );
        ...
        # 兼容升级：已有库（只有 U本位）补字段
        for ddl in (
            "ALTER TABLE basis_snapshot ADD COLUMN market TEXT",
            "ALTER TABLE basis_snapshot ADD COLUMN spot_symbol TEXT",
        ):
            try:
                conn.execute(ddl)
            except Exception:
                pass  # 字段已存在
        # 旧数据回填：market=UM, spot_symbol=pair（旧库 pair 就是 BTCUSDT）
        conn.execute("UPDATE basis_snapshot SET market='UM' WHERE market IS NULL")
        conn.execute("UPDATE basis_snapshot SET spot_symbol=pair WHERE spot_symbol IS NULL")
    ...
    with get_conn() as conn:
        conn.executemany("""
            INSERT INTO basis_snapshot (
                time, market, pair, spot_symbol, contract_type, contract_symbol,
