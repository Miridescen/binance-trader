...
def init_db():
    ...
    with get_conn() as conn:
        ...
        -- 24h 周期实盘开仓记录（子账号，结构同 open_log_8h，独立表以隔离监控/平仓）
        CREATE TABLE IF NOT EXISTS open_log_24h (
            id                  INTEGER PRIMARY KEY AUTOINCREMENT,
            open_anchor         TEXT,
            open_time           TEXT,
            close_time          TEXT,
        ...
        CREATE INDEX IF NOT EXISTS idx_open_log_24h_open_time ON open_log_24h(open_time);
...
# ── open_log_24h 操作（24h 周期实盘，子账号，batch 隔离）──────────────
# 结构/语义与上面的 *_8h 完全一致，仅换表名 open_log_8h → open_log_24h。

def insert_open_log_24h(row: dict) -> int:
    """开仓 INSERT，返回新行 id（其他字段后续 update 回填）"""
    row = dict(row)
    row.setdefault("open_anchor", None)
    with get_conn() as conn:
        cur = conn.execute("""
            INSERT INTO open_log_24h (open_anchor, open_time, close_time, symbol, side, entry_price,
                close_price, position_amt, leverage, unrealized_pnl, roe_pct,
                open_commission, close_commission, funding_fee, close_reason)
            VALUES (:open_anchor, :open_time, :close_time, :symbol, :side, :entry_price,
                :close_price, :position_amt, :leverage, :unrealized_pnl, :roe_pct,
...
def update_open_log_24h(row_id: int, fields: dict):
    if not fields:
        return
    cols = ", ".join(f"{k} = :{k}" for k in fields)
    with get_conn() as conn:
        conn.execute(f"UPDATE open_log_24h SET {cols} WHERE id = :id",
                     {**fields, "id": row_id})


def get_open_log_24h_unclosed() -> list[dict]:
    """所有未平仓行（close_time 为空）。按 (open_anchor, side) 分组即为各 batch。"""
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM open_log_24h WHERE close_time IS NULL OR close_time = '' ORDER BY id"
        ).fetchall()
        return [dict(r) for r in rows]


def get_open_log_24h_all() -> list[dict]:
    with get_conn() as conn:
        rows = conn.execute("SELECT * FROM open_log_24h ORDER BY id").fetchall()
        return [dict(r) for r in rows]


def get_open_log_24h_by_anchor_side(anchor_ts: str, side: str) -> list[dict]:
    """某个 batch（open_anchor + side）的所有行，容差到分钟。"""
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM open_log_24h WHERE open_anchor LIKE ? AND side = ? ORDER BY id",
            (f"{anchor_ts[:16]}%", side)
        ).fetchall()
        return [dict(r) for r in rows]


def get_open_log_24h_pending_writeback() -> list[dict]:
    """已下平仓单（close_time 已置）但真实数值还没回填（unrealized_pnl 为空）的行，
    用于回填重试（userTrades 结算有延迟时）。"""
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT * FROM open_log_24h WHERE close_time IS NOT NULL AND close_time != '' "
            "AND unrealized_pnl IS NULL ORDER BY id"
        ).fetchall()
        return [dict(r) for r in rows]
