#!/bin/bash
# 注册 binance-real-4h systemd 服务（4h 周期实盘策略）
# 用法：sudo bash start_real_4h.sh
set -e

PROJECT_DIR="$(cd "$(dirname "$0")" && pwd)"
PYTHON="$(which python3)"
USER="$(whoami)"
SERVICE_NAME="binance-real-4h"
...
[Unit]
Description=4h 周期实盘策略（涨幅+跌幅空，6 周期/天，4h 定平）
...
ExecStart=${PYTHON} ${PROJECT_DIR}/real_trade_4h.py
Restart=always
RestartSec=30
