...
- **模拟盘页显示「持仓中」的组及其实时浮盈**（`db.get_virtual_inprogress` / `api.py` / `VirtualLog4h.jsx`）
  - 此前窗口未结束且仍有未平单的组整组不显示，页面上看不到当前一轮的浮盈浮亏
  - 现在 `/api/virtual_inprogress` 同时返回两类进行中的组：① 已 +10u 提前平、等窗口结束；② 仍持仓中（新增）
  - 持仓中组的「实际 PnL」= 已平部分 + 未平部分浮盈；浮盈优先按实时标记价现算（与模拟盘同一口径 `_calc_pnl`，3x/10U），
    拿不到价时退回该单最近一次 5 分钟快照；行上带「浮动」标记，悬停可见来源与时间
  - 全市场标记价在 API 内做 10s 短缓存；行新增字段 `n_open / live_source / live_time`
  - 顶部指标改为三格：+10u 触发组 / 定平组 / 持仓中组（含合计浮盈）；持仓中的组不计入「+10u 触发组」
  - 面板头与 Tab 上的累计数仍只统计已收尾的组，口径不变

## 2026-09-09

- **24h 实盘止盈阈值 +50U → +65U**（`real_trade_24h.py` `TARGET_GROUP_PNL` 50→65；归一 +10 → +13）
  - 依据：24h 虚拟盘跌幅榜-空无过滤 69 窗（07-14→09-08）按实盘反推佣金 + 校准资金费**扣费后**的阈值扫描：
    +10 是 +8~+13 平台里最差一格（净 +112.8）；+9 全样本最高（+168.7）；+13 次之（+154.4）且前半/后半/最近四周各段均不弱、
    走前验证稳健；≥+20 与跑满 24h 在前半段亏损，属行情赌注，不采用
  - 折 5x 实盘 = +65U；止损 −150U 不变；预期每窗净增约 3U 量级，不是翻倍
  - 口径：以虚拟盘 5 分钟轨迹回放（同阈值下与实盘每窗差 ±1U 已验证）；高阈值受资金费（持仓越久越贵）结构性压制

## 2026-09-07

- **看板整站重新设计**（`frontend/`，功能与接口不变）
  - 深色顶栏 + 金色品牌块；去掉卡片套卡片，统一为「面板 + 标题栏」；等宽数字右对齐，盈亏行改为淡底色 + 左侧色条
  - 账户余额 / 保证金 / 持仓浮盈合并为账户头卡的三格指标条；自动开单 / 组内止损做成胶囊，危险态整颗变红
  - 样式收口：`theme.js`（antd 令牌）、`components/ui.jsx`（Panel / Stat / Chip / PnlCell）、`lib/fmt.js`（盈亏颜色与格式化），`App.css` 为唯一样式源
  - `vite.config.js` 代理目标可用环境变量 `API_PROXY` 覆盖（本地预览可指向线上 API），默认仍为本机 5001

- **实盘新增「组内止损」开关**（`real_trade_8h.py` / `real_trade_24h.py`，看板可切，**默认关**）
  - 触发：组内合计浮盈（税前，与止盈同口径）≤ −STOP_LOSS_PNL → 整组立即市价平，`close_reason=止损`
  - 阈值 = 3×每单保证金：8h **−30U**（10U/单），24h **−150U**（50U/单，5x）；两者 ≈ 名义 10% 逆行
  - 判定顺序：止盈 → 止损 → 定时平；开关每轮读一次，看板切换后 ≤30s 生效；启动与切换时打日志
  - 开关默认值单一事实源 `db.SWITCH_DEFAULTS`（real_* 默认开、stoploss_* 默认关；未知 key 一律 fail-closed）
  - 看板：各账户区块头新增「止损 −30U / −150U」开关；`fetchAll` 改为各请求独立结算（`Promise.allSettled`），
    止损/自动开单的武装状态不再因 `/api/realtime` 抖动而显示错误；首次拉到开关前灰显
  - 上线前经 5 视角对抗审查 + 反驳：1 项 major（上述看板显示问题）已修，其余为提示项

## 2026-07-30

- **修复 8h 实盘"隐形孤儿仓"bug（开仓漏记）** —— `real_trade_8h.py`
  - 现象：ESPUSDT 07-30 00:30 开了 490 空单，但 `open_log_8h` 没记录 → 平仓/监控逻辑不知道它存在 → 永不平仓，一路挂到手动清理
  - 根因：币安查单最终一致性（成交后立刻再查可能返回过时 NEW）。`insert_open_record_8h` 做了多余的二次 `get_order`，拿到过时的 NEW 就放弃入库
  - 修复：① ladder 已确认 FILLED 时把 order 对象直接传入，不再二次查单；② 入库不再因 status 过时而放弃，只看成交量；③ 新增 `reconcile_open_batch`——开仓周期结束后按账户实际持仓对账，凡"账户有仓但 DB 没记录"的自动补录，确保 交易所有仓=DB有记录
  - 说明：经真实成交核对，此前怀疑的 writeback 记假成交并不存在（#693 实际平仓干净），未改动 writeback

## 2026-07-29

- **8h 实盘止盈阈值 +10U → +16U**（`real_trade_8h.py` `TARGET_GROUP_PNL` 10→16，14:45 上线）
  - 依据：对虚拟盘 8h 跌幅空无过滤做阈值逐级扫描 + 滚动样本外（walk-forward）验证，
    +16U 在两种验证下都稳定优于 +10U（样本外 ~+40）；+10U 组平均峰值 +17、回落到 +11，
    说明 +10U 平仓偏早、把中段趋势让掉了，+16U 在半程峰值区落袋更优
  - **数据口径提示**：该结论来自虚拟盘（5 分钟快照对高阈值有轻微高估、无滑点），
    实盘真实增益可能小于虚拟盘数字；方向可信、幅度待实盘验证。上线后需盯实盘效果
  - `close_reason` 仍记为 `组内+10u`（作为“组内提前止盈”类别标签，兼容看板/统计）
  - **仅改实盘，虚拟盘（4/8/12/24h）阈值保持 +10U 不变**（作为对照基准）
  - 止损方向此前已验证：-10U/-15U/-20U/-25U/-30U 各档均降低盈利，不加
