...
账户隔离：
  - 本进程用子账号密钥。systemd 服务通过 EnvironmentFile=/root/binance-trader/.env.sub24h
    注入 BINANCE_API_KEY/SECRET；binance_client 的 load_dotenv(override=False) 不会覆盖
    已存在的环境变量，故本进程走子账号，其余服务照旧走 .env 主账号。

安全开关：
  - 环境变量 REAL_24H_LIVE=1 才会真正下单。未设置时为“观察模式”：
    照常选币 / 打印将要下的单 / 到点计算浮盈，但不下真单、不写库。
    用于上线前在服务器上核对选股与时间调度（并核实读到的是子账号余额）。
"""
...
log = logging.getLogger("real_24h")
...
def insert_open_record_24h(symbol: str, side_label: str, order_id: int, anchor_ts: str,
                           order: dict | None = None) -> int | None:
    """开仓成交后按实际成交数据 INSERT open_log_24h。
    ...
    new_id = db.insert_open_log_24h(row)
    log.info(f"  {symbol} INSERT open_log_24h#{new_id}: entry={avg_price} qty={executed} @ {open_time}")
    return new_id
...
def insert_open_from_position(symbol: str, side_label: str, anchor_ts: str) -> int | None:
    ...
    new_id = db.insert_open_log_24h(row)
    log.info(f"  {symbol} INSERT open_log_24h#{new_id}（按实际持仓）: entry={entry} qty={amt}")
    return new_id
...
def run_open_cycle(anchor: datetime):
    ...
    for attempt in range(2, MAX_LIMIT_ATTEMPTS + 1):
        ...
        for sym, data in pending.items():
            ...
            if status == "FILLED":
                log.info(f"  {sym} ✅ 已成交")
                insert_open_record_24h(sym, data["side_label"], data["orderId"], anchor_ts, order=order)
                continue
            ...
            if o2.get("status") == "FILLED":
                insert_open_record_24h(sym, data["side_label"], data["orderId"], anchor_ts, order=o2)
                continue
...
def reconcile_open_batch(targets: list, anchor_ts: str):
    ...
    for _, side_label in targets:
        if side_label not in recorded:
            recorded[side_label] = {r["symbol"] for r in db.get_open_log_24h_by_anchor_side(anchor_ts, side_label)}
    for sym, side_label in targets:
...
def _opened_already(anchor_ts: str, side_label: str) -> bool:
    return len(db.get_open_log_24h_by_anchor_side(anchor_ts, side_label)) > 0
...
def _mark_provisional_close(brows: list[dict], reason: str):
    ...
    for r in brows:
        db.update_open_log_24h(r["id"], {"close_time": now_ts, "close_reason": reason})
...
def close_batch_timed(brows: list[dict], symbol_info: dict, window_end: datetime):
    ...
    for r in brows:
        ...
        if actual:
            ...
            time.sleep(0.3)
            place_close_market(sym, actual, info)   # 用账户实际数量，避免 reduceOnly 数量过大被拒
            time.sleep(0.2)
...
def writeback_batch(brows: list[dict], reason: str):
    ...
    for r in brows:
        ...
        db.update_open_log_24h(r["id"], fields)
...
def monitor_batches():
    ...
    rows = db.get_open_log_24h_unclosed()
    if not rows:
        return
    batches: dict = {}
    for r in rows:
    ...
    for (anchor, side), brows in batches.items():
        ...
        if now >= window_end - timedelta(minutes=CLOSE_PREP_MIN):
            ...
            close_batch(brows, reason="24h_timed", market_now=False, window_end=window_end)
...
def check_force_close():
    ...
    复用 close_batch（市价平 → 残余清扫 → 临时标记 → 回填），与 +50U/定时平同一套收尾。"""
    if not LIVE:
        return
...
def main():
    db.init_db()
    log.info("24h 实盘策略启动  LIVE=%s" % LIVE)
    if not LIVE:
        log.warning("★ 观察模式（未设 REAL_24H_LIVE=1）：只选币/打印/计算，不下真单、不写库")
    ...
    while True:
        try:
            ...
            if LIVE:
                try:
                    pending = db.get_open_log_24h_pending_writeback()
                    if pending:
                        by_reason: dict = {}
                        for r in pending:
                            by_reason.setdefault(r.get("close_reason") or "24h_timed", []).append(r)
